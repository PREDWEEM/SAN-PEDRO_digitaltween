...
def test_reference_is_local_san_pedro_2025():
    reference = load_seasonal_reference(
        ROOT / "models" / "modelo_clusters_k3.pkl",
        excluded_years=("2010", "2015"), include_patterns=("san pedro",),
    )
    ...
    assert not reference["Campanas"].str.contains("2010|2015", case=False).any()

    assert reference.Campanas.eq("emrel sp 2025 san pedro.xlsx").all()
