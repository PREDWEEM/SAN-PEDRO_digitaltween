"""Interfaz PREDWEEM Digital Twin — San Pedro."""
...
st.markdown(
    ...
    <div class="hero">
      ...
      <h1>Gemelo Digital · Lolium San Pedro</h1>
...
with st.expander("Configuración del gemelo", expanded=True):
    ...
    st.caption(
        ...
        "(T0=24,5548 °C) · reservorio causal k=0,430257."
    )
...
try:
    calibration_profile = load_site_profile(CALIBRATION_DIR / "san_pedro_2026.json")
...
if source_option == "SIGA San Pedro + ECMWF operativa":
    st.caption(
        "Meteorología: observaciones SIGA–INTA A872890, puente provisional ECMWF "
        "IFS para fechas sin observación válida y pronóstico ECMWF ENS P50. "
...
with tab_state:
    ...
    st.caption("El interruptor controla la capa adicional 2026. La base conserva su calibración fisiológica 2025–2026; esta comparación es retrospectiva.")
    if calibration_audit["applied"] and calibration_profile is not None:
        validation = calibration_profile.get("validation", {})
        if validation.get("rmse_calibrated_units", 0) > validation.get("rmse_base_units", float("inf")):
            st.warning(
                "La calibración mejora el ajuste retrospectivo, pero empeora la evaluación "
    ...
    with left:
        ...
        st.write(
            ...
            f'El factor termohídrico está **{"por debajo de 0,50" if snapshot["thermoinhibited"] else "en 0,50 o más"}** (diagnóstico continuo).'
        )
        st.info(
...
with tab_calibration:
    st.subheader("Memoria local de San Pedro · campaña 2026")
    st.info(calibration_audit["reason"])
    if calibration_profile is not None:
        fit = calibration_profile["fit"]
        source = calibration_profile["source"]
        fit_table = pd.read_csv(CALIBRATION_DIR / "san_pedro_2026_fit.csv")
        st.write(
            ...
            'La columna plantas conserva la escala del adjunto; no informa superficie ni repeticiones. Se comparan '
        ...
        calibration_metrics[1].metric("RMSE base · ajuste", f'{fit["rmse_base_units"]:.1f} unidades del adjunto')
        calibration_metrics[2].metric("RMSE calibrado · ajuste", f'{fit["rmse_calibrated_units"]:.1f} unidades del adjunto')
        st.caption(
        ...
        for column, label, color in (
            ("Observado_Unidades", "Observado por intervalo", "#df5b3f"),
            ("Base_Unidades_ajuste", "PREDWEEM base", "#83938b"),
            ("Calibrado_Unidades_ajuste", "Calibración retrospectiva", "#9260bd"),
        ):
        ...
        fit_figure.update_layout(
            height=380, xaxis_title="Fin del intervalo", yaxis_title="Plantas por intervalo (escala del adjunto)",
            hovermode="x unified", plot_bgcolor="white", margin=dict(l=10, r=10, t=20, b=10),
        )
        st.plotly_chart(fit_figure, width="stretch")
        st.markdown("#### Evaluación retrospectiva por cortes")
        st.write(calibration_profile["validation"]["note"])
        validation = calibration_profile["validation"]
        validation_metrics = st.columns(2)
        validation_metrics[0].metric("RMSE base · evaluación por cortes", f'{validation["rmse_base_units"]:.2f} unidades del adjunto')
        validation_metrics[1].metric("RMSE calibrado · evaluación por cortes", f'{validation["rmse_calibrated_units"]:.2f} unidades del adjunto')
        if validation["rmse_calibrated_units"] > validation["rmse_base_units"]:
            st.warning(
                "La calibración empeora el RMSE de la evaluación por cortes. "
                "La mejora del ajuste retrospectivo no demuestra una mejora predictiva."
            )
        st.caption(
            'El motor base ya utilizó 2025 y 2026 para calibrarse; estos cortes no validan el sistema de forma independiente. '
        ...
        st.dataframe(
            pd.read_csv(CALIBRATION_DIR / "san_pedro_2026_holdout.csv"),
        ...
        st.download_button(
            ...
            (CALIBRATION_DIR / "san_pedro_2026_counts.csv").read_bytes(),
            "san_pedro_2026_counts.csv", "text/csv",
        )
        st.caption(
            'Los conteos se conservan para calibración en su escala original. Para asimilarlos como densidad, '
            'confirme primero la superficie y prepare una columna PLM2; el archivo plantas no se convierte automáticamente.'
        )
...
with tab_audit:
    ...
    audit_summary = pd.DataFrame(
        {
            "Variable": [
                ...
                "Reservorio de cohorte", "Referencia estacional local",
            ],
            "Valor": [
    ...
    export_columns = [
        "Fecha", "TMAX", "TMIN", "Prec", "FUENTE", "TIPODATO", "CALIDADDATO", "EMISION_UTC", "FECHA_EMISION",
