...
def _weekly_flow_trace(trace, cutoff, year_start, display_end, historical=False):
    ...
    for monday, group in weekly_flow_groups(trace.x, trace.y):
        ...
        if historical:
            ...
            source = "Potencial modelado del lote"
            if future_days:
    ...
    daily_figure.add_trace(
        go.Bar(
            ...
            hovertemplate=("%{x|%d/%m/%Y}<br>Flujo diario del gemelo: "
                           "%{y:.2f} % del total/día<br>"
                           "Potencial modelado del lote<extra></extra>"),
        ),
    )
    ...
    for figure in (daily_figure, cumulative_figure):
        ...
        if historical is not None:
            ...
            if not supported.empty:
                support_start = supported.iloc[0]
                if support_start > year_start:
                    figure.add_vrect(
                        x0=year_start, x1=support_start,
                        fillcolor="rgba(150,158,167,.08)", line_width=0, layer="below",
                        annotation_text="Sin referencia previa", annotation_position="top left",
