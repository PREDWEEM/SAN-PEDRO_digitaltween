...
def forecast_mask(frame: pd.DataFrame) -> pd.Series:
    ...
    type_column = next(
        (
            column
            for column in frame.columns
            if str(column).strip().lower() in {"tipodato", "tipo_dato", "data_type", "tipo"}
        ),
        None,
    )
...
def read_weather_file(source) -> pd.DataFrame:
    ...
    frame = pd.read_excel(source) if suffix in {".xlsx", ".xls"} else pd.read_csv(source)
    aliases = {"fecha": "Fecha", "date": "Fecha", "datetime": "Fecha",
...
def weather_source_label(df: pd.DataFrame) -> str:
    column = next((name for name in df if str(name).strip().lower() == "fuente"), None)
    if column is None:
        return "Archivo aportado"
    sources = [str(value) for value in df[column].dropna().unique()]
