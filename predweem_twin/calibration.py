...
def calibration_intervals(trajectory: pd.DataFrame, observations: pd.DataFrame):
    ...
    intervals = pd.DataFrame({
        ...
        "Observado_Unidades": observed,
        "EE_repeticiones_Unidades": se if "EE_repeticiones_PLM2" in obs else np.nan,
        "Sigma_ajuste_Unidades": sigma,
    })
...
def fit_site_calibration(trajectory, observations, *, site: str) -> tuple[dict, pd.DataFrame]:
    ...
    observed = comparison["Observado_Unidades"].to_numpy(float)
    sigma = comparison["Sigma_ajuste_Unidades"].to_numpy(float)
    ...
    comparison["Base_Unidades_ajuste"] = baseline
    comparison["Calibrado_Unidades_ajuste"] = best_predicted
    ...
    profile = {
        ...
        "fit": {
            ...
            "observed_window_total_units": float(observed.sum()),
            "rmse_base_units": float(np.sqrt(np.mean((baseline - observed)**2))),
            "rmse_calibrated_units": float(np.sqrt(np.mean((best_predicted - observed)**2))),
